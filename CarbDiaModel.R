...
# (1) microbial-mediated authigenic carbonate formation occurs via two prime pathways, ongoing dissolution an recrystallization with depth and as near-instantaneous authigenic carbonate seafloor crust formation 
...
PL$N <- 2000  # number of grid layers
...
PL$Db <- 0 # bioturbation intensity [cm2 yr-1] vandeVelde & Meysman 2016
PL$v  <- 0.2 # cm yr-1

PL$Db.grid <-setup.prop.1D(func = p.exp, grid = PL$grid, y.0 = PL$Db, y.inf = 0, x.L = 7.6)
...
# the 13C isotope composition of the product HCO of the following metabolic pathways; organoclastic Microbial Sulfate Reduction (MSR), Anaerobic Oxidation of Methane (AOM) and methanogenesis.
...
PL$Ks.SO4 <- 0.9  # SO4 limitation constant for consumption of OM by organoclastic sulfate reduction [umol cm-3] Meysman 2015
PL$Ks.AOM <- 1    # SO4 limitation constant for consumption of OM by methane-driven sulfate reduction [umol cm-3] Contreras 2013
...
# metabolism related carbon isotope fractionation factors, values taken from Irwin (1977) and Marshall (1992)
...
# reaction rate constants for authigenic carbonate production, Fantle and DePaolo 2007

PL$k.CCR_F <- 0.4     # calcium carbonate reactivity Fantle and Paolo 2007

PL$back.lime <-  5  # background carbon isotope value of Permian limestones, relative to VPDB  Schobben et al. 2016

# mineral isotope offset dissolved HCO and fixed in carbonate mineral lattice by temperature Emrich et al., 1970
...
PL$F.up.Carb <- PL$F.up.CH2O

PL$Frac <- 0.2                      # fraction authigenic carbonate (after Schobben et al., 2016)
...
PL$C.DIC.ow    <- 2.2 # DIC ocean water concentration [umol cm-3]
...
PL$C.SO4.ow    <- 4 # concentration SO4 in overlying water [umol cm-3] Permian = 4, modern = 28
...
# =============================================================================
  ...
  multi.model <- function (t,state,parameters) 
  {
    with(as.list(c(parameters)),{
      ...
      # -----------------------------------
      # Precipitation - reprecipitation isotope fractionation
      # -----------------------------------
      ...
      ddt.O2    <- Tran.O2    + Reac.O2
      ddt.SO4   <- Tran.SO4   + Reac.SO4
      ddt.CH4   <- Tran.CH4   + Reac.CH4
      ddt.HS    <- Tran.HS    + Reac.HS
      ddt.DIC   <- Tran.DIC   + Reac.DIC
      ddt.12CO2 <- Tran.12CO2 + Reac.12CO2
      ddt.13CO2 <- Tran.13CO2 + Reac.13CO2 
...
# testing for model robustness by checking for conservation of mass 
...
A<-ggplot(Sens.Pore, aes(y=depth, x=O2))+ 
  ...
  geom_point(aes(x=SO4), shape=1, color="blue")+ 
  geom_point(aes(x=HS), shape=1, color="purple")+ 
  geom_point(aes(x=CH4), shape=1, color="red")+ 

  ylab("depth (cm)")+
  xlab(expression(atop(paste(mu*mol~cm^-3))))+
  annotate("text", label=c("microbial-sulfate reduction"), y=5,  x=-0.35, size=3.5, angle=90)+
  annotate("text", label="O2", y=0,  x=0.5, size=3.5)+
  annotate("text", label="CH4", y=2,  x=0.35 , size=3.5)+
  annotate("text", label="HS", y=5,  x=2.5, size=3.5)+
  annotate("text", label="SO4", y=5,  x=1.5, size=3.5)+

  ggtitle("(a)"~"average OC flux")+
  theme(
...
# d13C profile with average OC accumulation, 500 [umol cm2- yr-1]
...
B<-ggplot(Sens.Pore, aes(y=depth, x=d13CDIC))+ 
  ...
  xlim(-16,16)+
  ylab("depth (cm)")+
  xlab(expression(atop(paste(delta^13*C~"(VPDB)"))))+
  annotate("text", label=c("microbial-sulfate reduction >"), y=60,  x=-15, size=3.5, angle=90)+
  annotate("text", label="DIC", y=15,  x=-6, size=3.5)+
  annotate("text", label="carbonate", y=30,  x=1.5 , size=3.5, angle=90)+
  
  
  ggtitle("(b)"~"average OC flux")+
  theme(
    axis.line.x = element_line(color = "black"), 
    axis.line.y = element_line(color = "black"),
    panel.grid.major = element_blank(),
...
C<-ggplot(Sens.Pore, aes(y=depth, x=O2))+ 
  ...
  annotate("text", label="CH4", y=7.5,  x=3 , size=3.5)+
  annotate("text", label="HS", y=5,  x=2, size=3.5)+
  annotate("text", label="SO4", y=1,  x=1, size=3.5)+
  
  ggtitle("(c)"~"high OC flux")+
  theme(
    axis.line.x = element_line(color = "black"), 
    axis.line.y = element_line(color = "black"),
    panel.grid.major = element_blank(),
...
D<-ggplot(Sens.Pore, aes(y=depth, x=d13CDIC))+ 
  ...
  annotate("text", label="DIC", y=15,  x=14, size=3.5)+
  annotate("text", label="carbonate", y=30,  x=3 , size=3.5, angle=90)+
  
  
  ggtitle("(d)"~"high OC flux" )+
  theme(
    axis.line.x = element_line(color = "black"), 
    axis.line.y = element_line(color = "black"),
    panel.grid.major = element_blank(),
...
# modern OC accumulation for model calibration, data from Muller and Suess (1979) 
...
S.P$sed.rate  <- c(0.0005, 0.0025, 0.2 , 1)  # sensitivity parameter sedimentation rate 
Out           <- list(NA,NA,NA,NA)           # empty list to collect results of sensitivity tests
...
#-------------------------------------------------------------------------------------------------------------------
# sensitivity of the model to the following changing variables with fraction of authigenic carbonate, dissolved oxygen 
# concentration of the overlying watercolumn, dissolved sulfate concentration of the overlying watercolumn and 
# sedimentation kept constant between runs at 0.2, 0.28, 4 and 0.0025, respectively. These latter values are considered 
# to be representative of a latest Permian carbonate depositional setting.
#-------------------------------------------------------------------------------------------------------------------   
          ...
          if(names(S.P)[j]=="sed.rate"){PL$v<- var[k]} else { PL$v<- 0.0025}         # sedimentation rate
...
# Storing of diagenetic stabalized carbonate d13C data
          ...
          d13C.sub2   <- (PL$Frac * median(d13C.auth[1:20]))+
...
Sens.v<-Out[[4]]   # sensitivity run, sedimentation rate
...
A<-ggplot(Sens.F, aes(x=FOC, y=Sens.F[,var[i]], group=var.sense, color=as.factor(var.sense) ))+geom_line()+
  ...
  ylab(expression(atop(paste(Delta^13*C[primary-bulk]~"(\211 VPDB)"))))+
...
D<-ggplot(Sens.v, aes(x=FOC, y=Sens.v[,var[i]], group=var.sense, color=as.factor(var.sense) ))+geom_line()+
...
FOC_sens<-arrangeGrob(A,B,C,D)

if(var[i]=="D13Cdia"){
ggsave("Figure5.pdf",FOC_sens, height= 16, width=16, units="cm" )} # saves produced graphs of solely ongoing diagenesis with depth

if(var[i]=="D13Cauth"){
  ggsave("Figure6.pdf",FOC_sens, height= 16, width=16, units="cm" )} # saves produced graphs of solely seafloor carbonate crust 
...
NPop<-100 # sample population size for generating spatial heterogeneous OC accumulation
...
[which((Sens.SO4[which(Sens.SO4$var.sense==4),])$D13C == max(((Sens.SO4[which(Sens.SO4$var.sense==4),])$D13C))
  ),] )$FOC 
...
#-------------------------------------------------------------------------------------------------------------------------        
      ...
      sigma <- log(1*factor2[t])         # log-transformed standard devation of OC flux
      
      PL$F.up.CH2O <- rlnorm(1,mu,sigma) # random generated OC flux from lognormal density distribution

#-------------------------------------------------------------------------------------------------------------------------  
      
      PL$v<- sed.rate[t] # sedimentation rate
      PL$v<-0.2
#-------------------------------------------------------------------------------------------------------------------------  
...
#-------------------------------------------------------------------------------------------------------------------------         
      ...
      # experiments, with a dissolved sulfate concentration of 4 (umol cm3-) in the overlying watercolumn
      
      PL$back.lime <- bulk.lime[t] + 
      (Sens.SO4[which(Sens.SO4$var.sense == 4 & 
      (abs(Sens.SO4$FOC-(org.acc*factor[t]))==
      min(abs(Sens.SO4$FOC-(org.acc*factor[t]))))),])$D13C # primary solid carbonate d13C 
        
      PL$back.fluid   <- PL$back.lime-PL$Delta_carb_bicarb # primary fluid carbonate d13C 
...
# instantaneous precipitated authigenic seafloor carbonate crust, calculated from DIC C isotope composition
        ...
        d13C.sub2[i]   <- median(d13C.auth[1:20]) # at 0.1 meter depth
        
# assigning a probability to authigenic carbonate formation and its impact on bulk rokc carbon isotope composition   
        ...
        }else{# the consecuvtive cycles of the timeseries
...
# assigning a probability to authigenic carbonate formation and its impact on bulk rokc carbon isotope composition         
    ...
    d13C.Dia  <- c(d13C.Dia,  d13C.sub1)
    d13C.Auth <- c(d13C.Auth, d13C.sub2)
    d13C.Bulk <- c(d13C.Bulk, d13C.sub3)
    d13C.sw   <- c(d13C.sw, d13C.sub4)
    FOC       <- c(FOC, d13C.sub5)
...
TimeSeries.sw.Iran <- data.frame(d13C.sw=d13C.sw[seq(1,10100, 100)], grid = ModelInput.Iran$x) # seawater DIC d13C


IPR<-function(x, seR=seq(0,1,0.025)){(quantile(x,seR))[40]-(quantile(x,seR))[2]} # interpercentile range [95%]
IQR<-function(x, seR=seq(0,1,0.25)){(quantile(x,seR))[4]-(quantile(x,seR))[2]}   # interquartile range [50%]

# Median trend lines, functions obtained from CarbTrends.R script
BootMedian.Iran<-BootSlide(n=999,data=TimeSeries.com.Iran$d13C.Bulk ,time=TimeSeries.com.Iran$grid, grid=grid, windowsize=windowsize, sampsize=10, method=median)
BootIQR.Iran<-BootSlide(n=999,data=TimeSeries.com.Iran$d13C.Bulk ,time=TimeSeries.com.Iran$grid, grid=grid, windowsize=windowsize, sampsize=10, method=IQR)
BootIQR_95.Iran<-BootSlide(n=999,data=TimeSeries.com.Iran$d13C.Bulk ,time=TimeSeries.com.Iran$grid, grid=grid, windowsize=windowsize, sampsize=10, method=IPR)
...
colorMed.Iran<-colorWeight(data=BootMedian.Iran, grid=grid)
colorIQR.Iran<-colorWeight(data=BootIQR.Iran, grid=grid)
colorIQR_95.Iran<-colorWeight(data=BootIQR_95.Iran, grid=grid)
...
ggplot(CI.sum.Iran, aes(x=x, y=M,  alpha=w3^3))+
  ...
  geom_vline(xintercept=(if(subject == "Iran"){6} else {5.9}), lty=2)+
